...
async function generateConfiguredLlm(options = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        const upstreamMessage = String(data?.error?.message || data?.message || text || '').trim();
        const errorMessage = /billing service temporarily unavailable|计费服务暂时不可用/i.test(upstreamMessage)
          ? '所选 LLM 的计费服务暂时不可用，请稍后重试或切换可用的 LLM 配置。'
          : (response.status === 502 && /bad gateway|failed to read origin site|origin site/i.test(upstreamMessage)
            ? 'LLM 上游网关暂时无法连接模型服务（HTTP 502），请检查该配置的 Base URL 或切换可用的 LLM 配置。'
            : (upstreamMessage || `LLM 上游 HTTP ${response.status}`));
      ...
      lastError = active.didTimeout()
        ...
        : error;
      const retryable = RETRYABLE_STATUSES.has(Number(lastError?.status))
        || /fetch failed|ECONNRESET|ETIMEDOUT|socket/i.test(String(lastError?.message || ''))
        // Some OpenAI-compatible gateways surface a transient billing outage
        // as HTTP 402 or a plain-text 500. Treat only the explicit temporary
        // outage wording as retryable; exhausted quota remains a hard error.
        || /billing service temporarily unavailable|计费服务暂时不可用/i.test(String(lastError?.message || ''));
