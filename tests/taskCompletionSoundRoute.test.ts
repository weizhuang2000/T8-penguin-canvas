...
test('settings route uploads a custom task failure audio file and can reset to default', async (t) => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 't8-task-failure-sound-'));
  t.after(() => {
    fs.rmSync(tmpDir, { recursive: true, force: true });
  });
  ...
  const base = `http://127.0.0.1:${server.address().port}/api/settings`;
  const form = new FormData();
  form.append('audio', new Blob([Uint8Array.from([0x49, 0x44, 0x33, 0x04])], { type: 'audio/mpeg' }), 'failed.mp3');

  const upload = await fetch(`${base}/task-failure-sound`, {
    method: 'POST',
    body: form,
  }).then((res) => res.json());
  assert.equal(upload.success, true);
  assert.equal(upload.data.mode, 'custom');
  assert.equal(upload.data.name, 'failed.mp3');
  assert.match(upload.data.url, /^\/api\/settings\/task-failure-sound\/file\?v=\d+/);

  const settings = await fetch(base).then((res) => res.json());
  assert.equal(settings.success, true);
  assert.equal(settings.data.taskFailureSound.mode, 'custom');
  assert.equal(settings.data.taskFailureSound.url, upload.data.url);

  const fileResponse = await fetch(`${base}/task-failure-sound/file`);
  assert.equal(fileResponse.status, 200);
  assert.equal(fileResponse.headers.get('content-type')?.startsWith('audio/mpeg'), true);
  assert.equal((await fileResponse.arrayBuffer()).byteLength, 4);

  const reset = await fetch(`${base}/task-failure-sound`, { method: 'DELETE' }).then((res) => res.json());
  assert.equal(reset.success, true);
  assert.equal(reset.data.mode, 'default');
  assert.equal(reset.data.url, '');

  const afterReset = await fetch(base).then((res) => res.json());
  assert.equal(afterReset.data.taskFailureSound.mode, 'default');
  assert.equal(afterReset.data.taskFailureSound.url, '');
});
