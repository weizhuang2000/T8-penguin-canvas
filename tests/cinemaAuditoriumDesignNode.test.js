...
test('cinema auditorium component exposes handles, shared material controls and generation services', () => {
  ...
  assert.match(source, /splitFlyingSuspendedRows/);
  assert.match(source, /倒扣半球穹幕\/顶部半圆屏/);
  assert.match(source, /圆形观众场地/);
  assert.match(source, /巨型凹弧幕\/飞行视景屏/);
  assert.match(source, /多层吊挂飞行座椅区/);
  ...
  assert.match(source, /nextVenueType === 'dome-cinema'/);
  assert.match(source, /screenType: 'dome-screen'/);
  assert.match(source, /slopeMode: 'reclined-dome'/);
  assert.match(source, /nextVenueType === 'flying-theater'/);
  assert.match(source, /screenType: 'wraparound-screen'/);
  assert.match(source, /specialEffects: \['motion-seats', 'wind', 'water-mist', 'scent'\]/);
...
test('cinema auditorium prompt presets cover special theaters and diagrams', async () => {
  ...
  assert.match(domeRender, /中间几排座椅数量最多|前后排逐渐减少/);
  const domePlan = mod.buildCinemaAuditoriumDrawingPrompt({ outputType: 'color-plan', venueType: 'dome-cinema', screenType: 'dome-screen', slopeMode: 'reclined-dome' });
  assert.match(domePlan, /穹幕影院彩色平面图/);
  assert.match(domePlan, /弧形座椅排/);
  assert.match(domePlan, /不得改成矩形影厅/);
  const flyingRender = mod.buildCinemaAuditoriumImagePrompt({ venueType: 'flying-theater', capacityMode: 'manual', seatCount: 96, colorPlanReferenceImages: ['/files/output/flying-plan.png'] });
  assert.match(flyingRender, /飞越式多层吊挂飞行影院|飞越式悬挂飞行影院/);
  assert.match(flyingRender, /巨大凹弧形飞行视景屏|半包裹弧幕/);
  assert.match(flyingRender, /后方装载平台|安全束缚/);
  assert.match(flyingRender, /不要画成零散独立小舱/);
  const flyingPlan = mod.buildCinemaAuditoriumDrawingPrompt({ outputType: 'color-plan', venueType: 'flying-theater' });
  assert.match(flyingPlan, /飞行影院彩色平面图/);
  assert.match(flyingPlan, /多层长排吊挂飞行座椅梁/);
  assert.match(flyingPlan, /不得画成普通地面座椅排布|零散独立座舱盒子/);
  const flyingPrinciple = mod.buildCinemaAuditoriumDrawingPrompt({ outputType: 'system-principle', venueType: 'flying-theater' });
  assert.match(flyingPrinciple, /show-control|PLC/);
  assert.match(flyingPrinciple, /钢结构大臂|升降臂|吊挂连杆/);
  assert.match(flyingPrinciple, /安全锁扣|急停|限位/);
