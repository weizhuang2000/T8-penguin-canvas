...
test('style transfer prompt uses original image as the only structure source', () => {
  ...
  assert.match(prompt, /@img1 是原始图像，@img2 是设计风格参考图/);
  assert.match(prompt, /必须以 @img1 作为空间结构、展品、文字、展示手段、构图、透视、比例、动线和具体内容的唯一依据/);
  assert.match(prompt, /仅从 @img2 提取展陈设计风格、色彩体系、材质语言/);
  assert.match(prompt, /原始图像是空间结构、展品、文字、展示手段、构图、视角、比例和动线/);
  assert.match(prompt, /不得从 @img2 复制空间架构、展品、文字、展示手段/);
  assert.match(prompt, /执行方式：把@img2 提取展陈设计风格、色彩体系、材质语言、表面肌理、光泽关系、灯光氛围和细部质感迁移到@img1已有的墙面、地面、展台、展柜、装置、导视、标题字、灯带和装饰表面上/);
...
test('style transfer prompt uses shared color material preset without reference role', () => {
  ...
  assert.doesNotMatch(prompt, /@img2/);
