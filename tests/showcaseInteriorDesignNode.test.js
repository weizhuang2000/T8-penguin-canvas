...
test('showcase interior design component wires shared controls and generation services', () => {
  ...
  assert.match(source, /materials=\{mentionMaterials\}/);
  assert.match(source, /supplement: resolvedSupplement/);
  assert.match(source, /mentionMaterials: Material\[\] = useMemo\(\(\) => exhibitImages\.map/);
  assert.match(source, /kind: 'image'/);
  assert.match(source, /origin: 'upstream'/);
  ...
  assert.match(source, /按输入尺寸/);
  assert.match(source, /模型按价值/);
  assert.match(source, /按文物级别/);
  assert.match(source, /HERITAGE_LEVEL_OPTIONS/);
  assert.match(source, /一级/);
  assert.match(source, /二级/);
  assert.match(source, /三级/);
  ...
  assert.match(source, /disabled=\{isReadonly \|\| busy \|\| layoutMode === 'manual' \|\| supportHeightMode === 'heritage-level'\}/);
  ...
  assert.match(source, /drag\.item\.widthMm \+ dxMm/);
  assert.match(source, /drag\.item\.heightMm \+ dyMm/);
  assert.match(source, /window\.addEventListener\('pointerup', endDrag/);
  assert.match(source, /手动排版模式/);
  assert.match(source, /layoutMode === 'auto'/);
  assert.doesNotMatch(source, /自动尺寸模式暂不可用/);
  ...
  assert.match(source, /d\.aspectRatioSource === 'manual'/);
  assert.match(source, /aspectRatioSource: 'manual'/);
  assert.match(source, /按展柜宽高自动建议/);
