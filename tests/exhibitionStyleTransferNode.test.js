...
test('exhibition style transfer node exposes image ports and shared controls', () => {
  ...
  assert.match(read('src/components/Canvas.tsx'), /targetType === 'exhibition-style-transfer' && \(handle === 'original-image' \|\| handle === 'style-reference'\)[\s\S]*return \[handle\]/);
  ...
  assert.match(source, /mentionToken: '@img1'/);
  assert.match(source, /mentionToken: '@img2'/);
  ...
  const mentionSource = read('src/components/nodes/mediaMentions.ts');
  assert.match(mentionSource, /@img\\d\+\\b/);
  assert.doesNotMatch(mentionSource, /@图片/);
});
