        ...
        <div className="fixed inset-0 z-[60] bg-black/55 flex items-center justify-center p-4" onClick={() => setInfoItem(null)}>
          ...
          >
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex items-start gap-3">
                <span className={`w-16 shrink-0 ${subtle}`}>存储空间</span>
                <span className="min-w-0 flex-1 break-words">
                  {infoItem.storageSpaceId === 'primary' || !infoItem.storageSpaceId ? '当前服务器' : infoItem.storageSpaceId}
                  {infoItem.storageFallbackFrom ? `（已从 ${infoItem.storageFallbackFrom} 回落）` : ''}
                </span>
              </div>
            </div>
          </div>
