...
const ExhibitionTextImageLoopNode = ({ id, data, selected }: NodeProps) => {
  ...
  return (
    <div data-exhibition-compact-node-type="exhibition-text-image-loop" className="relative" style={containerStyle}>
      <Handle id="text" type="target" position={Position.Left} className="!h-3 !w-3 !border-0 t8-exhibition-handle--text" style={{ top: '34%', left: -6, background: EXHIBITION_TEXT_HANDLE_COLOR }} title="输入：文本素材集" />
      <Handle id="image" type="target" position={Position.Left} className="!h-3 !w-3 !border-0 t8-exhibition-handle--image" style={{ top: '58%', left: -6, background: EXHIBITION_IMAGE_HANDLE_COLOR }} title="输入：图像素材集" />
      <Handle id="text" type="source" position={Position.Right} className="!h-3 !w-3 !border-0 t8-exhibition-handle--text" style={{ top: '38%', right: -6, background: EXHIBITION_TEXT_HANDLE_COLOR }} title="输出：文本" />
      <Handle id="image" type="source" position={Position.Right} className="!h-3 !w-3 !border-0 t8-exhibition-handle--image" style={{ top: '62%', right: -6, background: EXHIBITION_IMAGE_HANDLE_COLOR }} title="输出：图像" />
      ...
      <div className="nodrag nopan" style={{ padding: 10 }} onMouseDown={(event) => event.stopPropagation()} onWheelCapture={(event) => event.stopPropagation()}>
        <div data-exhibition-compact-section="run" data-exhibition-compact-item="main" style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
        ...
        <div data-exhibition-compact-section="pairing" data-exhibition-compact-item="main" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 5, marginBottom: 9 }}>
        ...
        <div data-exhibition-compact-section="input" data-exhibition-compact-item="main" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 8 }}>
        ...
        <div data-exhibition-compact-section="status" data-exhibition-compact-item="main" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {status === 'running' ? (
