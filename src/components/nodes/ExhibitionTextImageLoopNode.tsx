...
const ExhibitionTextImageLoopNode = ({ id, data, selected }: NodeProps) => {
  ...
  return (
    <div data-exhibition-compact-node-type="exhibition-text-image-loop" className="relative" style={containerStyle}>
      <Handle id="text" type="target" position={Position.Left} className="!h-3 !w-3 !border-0" style={{ top: '34%', left: -6, background: EXHIBITION_TEXT_HANDLE_COLOR }} title="输入：文本素材集" />
      <Handle id="image" type="target" position={Position.Left} className="!h-3 !w-3 !border-0" style={{ top: '58%', left: -6, background: EXHIBITION_IMAGE_HANDLE_COLOR }} title="输入：图像素材集" />
      <Handle id="text" type="source" position={Position.Right} className="!h-3 !w-3 !border-0" style={{ top: '38%', right: -6, background: EXHIBITION_TEXT_HANDLE_COLOR }} title="输出：文本" />
      <Handle id="image" type="source" position={Position.Right} className="!h-3 !w-3 !border-0" style={{ top: '62%', right: -6, background: EXHIBITION_IMAGE_HANDLE_COLOR }} title="输出：图像" />
