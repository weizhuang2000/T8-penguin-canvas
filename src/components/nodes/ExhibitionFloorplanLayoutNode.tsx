  ...
  return <div className="relative w-full rounded-xl border border-cyan-400/25 bg-slate-950/95 text-white shadow-xl">
    ...
    <div className="grid grid-cols-[230px_1fr] gap-2 p-2">
      ...
      <div className="min-w-0 space-y-2">
        ...
        {d.render?.imageUrl && <div className="rounded border border-white/10 p-1"><div className="mb-1 text-[9px] text-white/55">AI 表现层 {d.render.stale ? '（布局已变化，当前预览已过期）' : ''}</div><SmartImage className={`max-h-48 w-full rounded object-contain ${d.render.stale ? 'opacity-40' : ''}`} src={d.render.imageUrl} alt="AI表现层" thumbSize={360} /></div>}
      </div>
    </div>
