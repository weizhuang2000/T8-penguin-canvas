...
const ExhibitionOutlineSplitNode = ({ id, data, selected }: NodeProps) => {
  ...
  return (
    <div data-exhibition-compact-node-type="exhibition-outline-split" className={`t8-node relative w-[620px] transition-all ${selected ? 'ring-2 ring-cyan-300' : ''}`}>
      <Handle type="target" position={Position.Left} className="!border-0 t8-exhibition-handle--text" style={{ background: EXHIBITION_TEXT_HANDLE_COLOR }} title="输入：展陈资料文档文本（DOCX/PDF/TXT 或直接粘贴）" />
      <Handle id={OUTLINE_TEXT_HANDLE} type="source" position={Position.Right} className="!border-0 t8-exhibition-handle--text" style={{ top: '42%', background: EXHIBITION_TEXT_HANDLE_COLOR }} title="输出：拆分后的大纲文本段（可连接多个下游节点）" />
      <Handle id={OUTLINE_IMAGE_HANDLE} type="source" position={Position.Right} className="!border-0 t8-exhibition-handle--image" style={{ top: '58%', background: EXHIBITION_IMAGE_HANDLE_COLOR }} title="输出：拆分过程中提取的配图（如有）" />
