    ...
    <div className="relative p-4 text-sm" style={rootStyle} data-fhl-image-gen-root>
      ...
      <div className="nowheel max-h-[760px] space-y-3 overflow-y-auto pr-1" onWheelCapture={(event) => event.stopPropagation()}>
        ...
        <section className="rounded-xl p-3" style={fieldStyle} data-fhl-job-status>
          ...
          {job?.tasks?.length ? <div className="mt-2 max-h-32 space-y-1 overflow-y-auto text-[10px]">{job.tasks.slice(-12).map((task) => <div key={task.id} className="flex gap-2"><span className={task.status === 'success' ? 'text-emerald-400' : task.status === 'failed' ? 'text-rose-400' : 'text-cyan-400'}>{task.status}</span><span className="min-w-0 flex-1 truncate">{task.templateLabel || task.id}</span><span>{task.workerName || ''}</span><span>{task.attempts ? `×${task.attempts}` : ''}</span></div>)}</div> : null}
