...
const ExhibitionRecolorNode = ({ id, data, selected }: NodeProps) => {
  ...
  return (
    ...
    >
      <Handle id="original-image" type="target" position={Position.Left} className="!h-3 !w-3 !border-0 t8-exhibition-handle--image" style={{ top: '28%', background: EXHIBITION_IMAGE_HANDLE_COLOR }} title="输入：原始图像" />
      <Handle type="source" position={Position.Right} className="!border-0 t8-exhibition-handle--image" style={{ background: EXHIBITION_IMAGE_HANDLE_COLOR }} title="输出：换色结果图像" />
      ...
      <div className="nodrag nopan max-h-[760px] space-y-2 overflow-y-auto p-2.5" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <section data-exhibition-compact-section="model" className="space-y-2 rounded border border-white/10 bg-white/[0.035] p-2">
          <FhlImageModuleControls compact nodeId={id} data={d} update={update} busy={busy} isReadonly={isReadonly} referenceCount={1} />
