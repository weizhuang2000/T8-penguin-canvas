...
import { Boxes, Image as ImageIcon, Layers, Loader2, Play, RotateCw, ShieldCheck, Trash2, X } from 'lucide-react';
import { EXHIBITION_IMAGE_HANDLE_COLOR } from '../../config/portTypes';
import { IMAGE_MODELS } from '../../providers/models';
import { generateExternalImage, generateLlm, queryExternalImageStatus, queryImageStatus, recordGenerationHistory, submitImageAsync } from '../../services/generation';
...
import {
  ...
  validationCorrectionText,
  type ReverseIsometricLayoutItem,
  type ReverseIsometricValidationReport,
...
const ReverseIsometricDesignNode = ({ id, data, selected }: NodeProps) => {
  ...
  const configuredLlmModel = useApiKeysStore((state) => state.settings.llmModel)?.trim() || 'gpt-4o-mini';
  ...
  const llmOptions = useMemo(() => {
    ...
    return saved.length ? saved : [{ id: 'default', label: '默认 LLM', model: configuredLlmModel }];
  }, [configuredLlmModel, llmConfigs]);
  const activeLlm = llmOptions.find((item) => item.id === d.llmKeyId) || llmOptions.find((item) => item.isDefault) || llmOptions[0];
  const llmModel = activeLlm?.model || d.llmModel || configuredLlmModel;
  ...
  const generateCandidate = useCallback(async (prompt: string, references: string[], runSeed: number): Promise<string> => {
    if (isExternal) {
      ...
      for (let index = 0; !response.imageUrls?.length && response.taskId && index < MAX_POLLS; index += 1) {
        ...
        response = await queryExternalImageStatus({ providerId: providerSelection.provider.id, providerModel: externalModel, taskId: response.taskId, outputFormat });
    ...
    const submit = await submitImageAsync({ model: modelDef.id, apiModel, paramKind: modelDef.paramKind, prompt, aspect_ratio: aspectRatio, image_size: sizeLevel, images: references, n: 1, outputFormat, seed: runSeed });
    if (submit.urls?.[0]) return submit.urls[0];
    if (!submit.taskId) throw new Error('未获取到生图任务 ID');
    for (let index = 0; index < 1800; index += 1) {
      if (pollAbortRef.current) throw new Error('任务已取消');
      await new Promise((resolve) => setTimeout(resolve, 2000));
      const result = await queryImageStatus(submit.taskId, apiModel, outputFormat);
  ...
  }, [apiModel, aspectRatio, d.providerParams, externalModel, isExternal, modelDef.id, modelDef.paramKind, outputFormat, providerSelection.provider, sizeLevel, update]);

  const validateCandidate = useCallback(async (candidateUrl: string): Promise<ReverseIsometricValidationReport> => {
    const response = await generateLlm({
      model: llmModel,
      llmKeyId: activeLlm?.id === 'default' ? undefined : activeLlm?.id,
      temperature: 0,
      max_tokens: 1200,
      messages: [
    ...
    return parseReverseIsometricValidationReport(response.content);
  }, [activeLlm?.id, llmModel, planImage]);
  ...
  const runGenerate = useCallback(async () => {
    ...
    if (!llmModel) throw new Error('请选择支持图片输入的结构校验模型');
    ...
    try {
      ...
      const validationRun = await runReverseIsometricValidationLoop({
        ...
        onPhase: ({ phase, attempt }) => update(phase === 'generate'
          ? { status: 'generating', progress: attempt === 0 ? '正在生成轴侧候选图...' : '结构未通过，正在纠错重试...' }
          : { status: 'validating', progress: `正在进行第 ${attempt + 1} 次结构一致性校验...` }),
      });
      const { candidate, report, prompt: finalPrompt } = validationRun;
      update({ validationReport: report });
      if (!validationRun.passed || !report?.pass || !candidate) {
        const detail = validationCorrectionText(report || { pass: false, confidence: 0, violations: [] });
        update({ ...previousOutput, status: 'error', progress: '', error: `结构校验连续两次未通过${detail ? `：${detail}` : ''}`, validationReport: report, manualLayoutReferenceImage: layoutReference });
        throw new Error('结构校验连续两次未通过，候选图已拒绝发布');
      }
      update({ status: 'success', progress: '100%', error: '', imageUrl: candidate, imageUrls: [candidate], urls: [candidate], outputText: finalPrompt, text: finalPrompt, prompt: finalPrompt, lastPrompt: finalPrompt, lastSeed: runSeed, referenceImages: references, manualLayoutReferenceImage: layoutReference, validationReport: report });
      await recordGenerationHistory([{ url: candidate, kind: 'image', title: '反推轴侧图', prompt: finalPrompt, model: isExternal ? externalModel : apiModel, seed: runSeed }], { canvasId: activeCanvasId, sourceNodeId: id, sourceNodeType: 'reverse-isometric-design', nodeTitle: '反推轴侧', outputTitle: '反推轴侧图', prompt: finalPrompt, model: isExternal ? externalModel : apiModel, seed: runSeed }).catch((error) => {
        logBus.warn(`反推轴侧结果已发布，但历史登记失败：${error?.message || error}`, `reverse-isometric:${id.slice(0, 6)}`);
      });
      logBus.success('反推轴侧生成完成并通过结构校验', `reverse-isometric:${id.slice(0, 6)}`);
      taskCompletionSound.notifyComplete(id, 'image');
    } catch (error: any) {
      if (String(error?.message || '').includes('候选图已拒绝发布')) throw error;
  ...
  return <div data-exhibition-compact-node-type="reverse-isometric-design" className={`relative w-[520px] rounded-xl border bg-zinc-950 text-white shadow-2xl ${selected ? 'border-cyan-300/70' : 'border-white/10'}`}>
    ...
    <Handle type="source" position={Position.Right} className="!h-3 !w-3 !border-0 t8-exhibition-handle--image" style={{ background: EXHIBITION_IMAGE_HANDLE_COLOR }} title="输出：通过结构校验的展陈轴侧图" />
    <header className="flex items-center gap-2 border-b border-white/10 px-3 py-2"><Boxes size={16} className="text-cyan-300" /><div className="min-w-0 flex-1"><div className="text-sm font-semibold text-cyan-100">反推轴侧</div><div className="truncate text-[10px] text-white/45">锁定平面结构 · 手动展项排版 · Vision 强制校验</div></div><NodeHelpButton nodeType="reverse-isometric-design" /></header>
    <div className="nodrag nopan max-h-[780px] space-y-2 overflow-y-auto p-2.5" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <section data-exhibition-compact-section="model" data-exhibition-compact-item="main" className="space-y-2 rounded border border-white/10 bg-white/[0.035] p-2">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <label className="col-span-2 space-y-1"><span className="text-[10px] text-white/55">结构校验模型（必须支持 Vision）</span><select className={FIELD} value={activeLlm?.id || ''} disabled={isReadonly || busy} onChange={(e) => { const next = llmOptions.find((item) => item.id === e.target.value) || llmOptions[0]; update({ llmKeyId: next?.id || '', llmModel: next?.model || configuredLlmModel }); }}>{llmOptions.map((item) => <option key={item.id} value={item.id}>{item.label || item.id} · {item.model || configuredLlmModel}</option>)}</select></label>
        </div>
      </section>
      {(d.progress || d.error || d.validationReport) && <section data-exhibition-compact-section="validation" data-exhibition-compact-item="main" className="space-y-1 rounded border border-cyan-300/20 bg-cyan-300/10 p-2">{d.progress && <div className="text-[10px] text-cyan-100">{d.progress}</div>}{d.error && <div className="text-[10px] text-rose-200">{d.error}</div>}{d.validationReport && <div className={`flex items-center gap-1 text-[10px] ${d.validationReport.pass ? 'text-emerald-200' : 'text-amber-200'}`}><ShieldCheck size={12} />结构校验：{d.validationReport.pass ? '通过' : '未通过'} · 置信度 {Math.round(Number(d.validationReport.confidence || 0) * 100)}%</div>}</section>}
