...
import type { SculptureReliefMaterialItem } from '../../services/api';
...
type DraftMaterial = SculptureReliefMaterialItem & { draftId: string };

interface Props {
  open: boolean;
  materials: SculptureReliefMaterialItem[];
  saving?: boolean;
  error?: string;
  onClose: () => void;
  onSave: (materials: SculptureReliefMaterialItem[]) => void | Promise<void>;
}

function makeDraft(item: Partial<SculptureReliefMaterialItem>, index: number): DraftMaterial {
...
function toSave(item: DraftMaterial, order: number): SculptureReliefMaterialItem {
  return {
    id: item.id,
    category: String(item.category || '默认').trim() || '默认',
...
export default function SculptureReliefMaterialEditorModal({
  open,
  materials,
  saving = false,
  error = '',
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/60 p-6">
      <div className="flex max-h-[86vh] w-[980px] flex-col rounded-xl border border-white/10 bg-zinc-950 text-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div>
            <div className="text-sm font-semibold">雕塑/浮雕主材质选项</div>
        ...
        <div className="min-h-0 flex-1 overflow-auto p-4">
          <div className="space-y-2">
            {drafts.map((item, index) => (
              <div key={item.draftId} className="grid grid-cols-[88px_120px_1fr_1fr_1fr_1fr_34px] gap-2 rounded border border-white/10 bg-white/[0.035] p-2">
                ...
                <input className={FIELD} value={item.prompt || ''} disabled={saving} placeholder="Prompt 补充" onChange={(e) => updateDraft(item.draftId, { prompt: e.target.value })} />
