...
## 输入与模型
- 左侧图像端口可连接上传素材、输出素材或图像素材集，也可在节点内上传本地图片
- 多图时图 1 作为主体，其余图片作为补充参考，拖动缩略图可调整顺序
- 识图模型只能从 API 设置中的 LLM 独立配置选择，模型名称随配置固定

## 细节强度
- **简洁**：只保留主体、构图、场景和风格等决定画面的信息
- **标准**：均衡描述主体、镜头、环境、光色、材质与氛围（默认）
- **详细**：进一步补充空间层次、材质表现和微观细节
- **极致**：最大程度复现所有可见元素及其视觉关系

## 输出
- 可选择简体中文或英文
- 结果写入标准文本输出，可直接连接图像节点的 GPT Image 2 模型
- 输出不包含 Midjourney 参数、Stable Diffusion 权重、JSON、Markdown 或解释文字
- 生成后可在节点内继续编辑、复制或保存到提示词模板库
`,
