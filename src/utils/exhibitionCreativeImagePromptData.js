...
function exhibitionCreativeColorPaletteText({ colorMaterial, colorMaterialPriorityMode, colorMaterialReferenceTone, hasColorMaterialReferenceImage }) {
  if (colorMaterial) return colorMaterial;
  if (hasColorMaterialReferenceImage) {
...
export function buildExhibitionCreativeImagePrompt(values = {}) {
  ...
  const hasColorMaterialReferenceImage = values.hasColorMaterialReferenceImage === true;
  const colorMaterial = hasColorMaterialReferenceImage ? '' : cleanExhibitionCreativeText(values.colorMaterial, 1000);
  const colorMaterialOverride = cleanExhibitionCreativeText(values.colorMaterial, 1000);
  ...
  const lines = [
    ...
    `Color palette: ${exhibitionCreativeColorPaletteText({ colorMaterial: colorMaterialOverride || colorMaterial, colorMaterialPriorityMode, colorMaterialReferenceTone, hasColorMaterialReferenceImage })}`,
