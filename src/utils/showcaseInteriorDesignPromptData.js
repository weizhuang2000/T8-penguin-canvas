...
function showcaseScaleRuleText(style) {
  const s = normalizeShowcaseStyle(style);
  return s.hasCap
    ? '生成缩放规则：生图时展品本体显示高度必须按用户设定高度生成；展柜宽度、底座高度、玻璃区高度、柜帽高度和柜体总高度保持设定尺寸不变。'
    : '生成缩放规则：生图时展品本体显示高度必须按用户设定高度生成；展柜宽度、底座高度、玻璃区高度和柜体总高度保持设定尺寸不变；顶部保持透明玻璃顶，不安装任何灯具、灯带或射灯。';
}
...
function exhibitItemsText(items, style, values = {}) {
  ...
  if (layoutMode === 'manual') {
    ...
    const lines = [
      ...
      '不要套用自动尺寸模式中的“高度 mm”规则；不要为了画面美观擅自重新放大、缩小或改动展品位置。',
      s.hasCap
  ...
  const lines = [
    ...
    '标注规则：如果开启尺寸标注，展品、展柜和构件的标注文字必须标注用户设定尺寸；展品画面高度与标注高度保持一致。',
    '高度定义：高度只指展品本体的可见垂直高度，不包含托台、托盘、标签牌、底座、支架、阴影、留白或说明文字。',
    '最终展品本体必须严格按设定高度形成真实显示比例范围，不得为了构图、焦点或视觉美观而随意放大或缩小。',
  ...
  normalized.forEach((item, index) => {
    ...
    lines.push(`${index + 1}. ${mentionToken} ${item.label}：设定高度 ${item.heightMm} mm；参考图 URL：${item.url || '[上游展品图]'}`);
    ...
    lines.push(`   标注校验：如输出尺寸标注，展品 ${index + 1} 标注为 ${item.heightMm} mm。`);
    lines.push(`   上限约束：展品 ${index + 1} 的本体可见高度不得超过其设定高度；如果不确定，宁可略小，不要放大。`);
  });
  ...
  return [
    ...
    values.dimensionMarksEnabled === true
      ? isManualLayout
        ...
        : '尺寸标注：开启。输出中可加入清晰的工程尺寸标注、毫米单位和关键高度/宽度标注，但文字必须简洁、整洁，像方案图标注。所有标注必须使用用户设定尺寸：展柜尺寸不变，展品标注为设定高度。'
      : isManualLayout
        ? '尺寸标注：关闭。不要绘制尺寸线、毫米数字、红色测量标注、工程尺或标注符号；展品位置和显示大小仍必须严格按第 1 张手动排版合成图。'
        : '尺寸标注：关闭。不要绘制尺寸线、毫米数字、红色测量标注、工程尺或标注符号，但仍要按给定尺寸比例生成；展品视觉高度按设定高度，展柜尺寸不变。',
    explodedViewText,
  ].join('\n');
}
