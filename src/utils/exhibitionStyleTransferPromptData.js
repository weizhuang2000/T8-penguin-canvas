...
function styleSourceText(mode, values) {
  if (mode === 'style-reference') {
    return [
      '图像引用：@img1 是原始图像，@img2 是设计风格参考图。',
      '必须以 @img1 作为空间结构、展品、文字、展示手段、构图、透视、比例、动线和具体内容的唯一依据。',
      '风格来源：仅从 @img2 提取展陈设计风格、色彩体系、材质语言、表面肌理、光泽关系、灯光氛围和细部质感。',
      '不得从 @img2 复制空间架构、展品、文字、展示手段、构图、透视、比例、动线或具体内容。',
    ].filter(Boolean).join('\n');
  }
...
function executionText(mode) {
  if (mode === 'style-reference') {
    return '执行方式：把@img2 提取展陈设计风格、色彩体系、材质语言、表面肌理、光泽关系、灯光氛围和细部质感迁移到@img1已有的墙面、地面、展台、展柜、装置、导视、标题字、灯带和装饰表面上；只做材质与风格层面的替换，不移动、不增删、不重构任何核心元素。';
  }
...
export function buildExhibitionStyleTransferPrompt(values = {}) {
  ...
  const lines = [
    ...
    '',
    '最高优先级：原始图像是空间结构、展品、文字、展示手段、构图、视角、比例和动线的唯一依据；同时也是开口、墙体、顶面、地面、展柜、展台和主要内容的唯一依据。',
    '',
