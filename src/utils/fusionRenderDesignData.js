...
export function buildFusionRenderPrompt({
  ...
  const presetText = String(colorMaterialPresetText || '').trim();
  const manualColorMaterial = String(colorMaterial || '').trim();
  ...
  const referenceRoles = hasSpaceReference
    ? `@img1 是空间参考图，只用于约束空间几何、面积感、平面长宽比例、面积与高度比例、墙柱开口、边界和纵深；严禁从中采用设计语言、色彩、材质、灯光或环境氛围。${exhibitCount > 0 ? `@img${firstExhibitIndex} 至 @img${lastExhibitIndex} 是需要作为素材融入展厅的展项原始外观参考。` : ''}`
  ...
  return [
    ...
    subjectConstraint,
    '相机视角：相机必须保持约 1.4–1.6 米的较低正常人眼高度，使用平视或轻微仰视的自然广角透视，不得采用架高机位、俯拍、鸟瞰或接近轴侧的视角。不要为了让所有展项同时完整出现在画面中而提高相机、扩大俯角或压缩空间纵深；不得输出俯视平面图、轴侧图或高空鸟瞰图。',
    ...
    '展项布置：识别各展项的真实形态、用途、观看面和尺度，根据展馆类型、展厅主体、空间参考图提供的纯空间几何及展厅净高自动完成专业展陈布置。保持各展项的核心识别特征、色彩、材质和合理相对尺度，保证通道、观看距离、安全边界和主次层级自然可信。',
    hasSpaceReference
      ? `空间参考图硬约束：严格保持 @img1 的空间边界、平面长宽比例、总体面积感、面积与高度的相对比例、墙柱关系、开口关系、空间纵深和主要结构拓扑。节点设置的净高 ${safeHeight} mm 是唯一绝对高度基准，必须严格按该数值重建空间；在不改变参考图平面面积比例的前提下，将参考图的面积—高度关系按 ${safeHeight} mm 等比换算。不得擅自压低、抬高、拉长、压缩或扩大空间。只保留空间几何约束，必须完全忽略 @img1 的设计语言、装饰风格、色彩、材质、灯光、曝光、环境氛围、原有展项、人物和文字。`
      : '空间构建：没有空间参考图时，根据展馆类型、展厅主体及净高创建一个完整、自然、可施工的展厅空间。',
    `展厅净高绝对硬约束：${safeHeight} mm。该数值优先于空间参考图中的视觉高度、透视错觉和原始建筑高度；必须严格据此建立墙体、柱体、顶部、门洞、展项和人物的尺度关系，不得自行修改高度。`,
    floorConstraint,
    ceilingConstraint,
