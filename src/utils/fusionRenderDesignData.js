...
export function buildFusionRenderPrompt({
  ...
  const colorMaterialConstraint = presetText
    ? `色彩与材质预设：严格采用共享预设“${presetText}”。该预设优先控制墙面、顶部、地面、展台、辅助结构、灯光色温和整体空间材料语言；不要被空间参考图或展项自身背景色覆盖。`
    : manualColorMaterial
      ? `色彩与材质补充：${manualColorMaterial}。将这些要求统一应用于展厅环境与辅助结构，同时保持展项本体的识别色材。`
      : '色彩与材质：未选择共享预设或手动补充时，根据展馆类型、展厅主体、空间参考图和全部展项自动建立统一协调的色彩与材质体系。';
  ...
  return [
    ...
    '展项布置：识别各展项的真实形态、用途、观看面和尺度，根据展馆类型、展厅主体、空间参考图及展厅净高自动完成专业展陈布置。保持各展项的核心识别特征、色彩、材质和合理相对尺度，保证通道、观看距离、安全边界和主次层级自然可信。',
    hasSpaceReference
      ? '空间参考图使用规则：借鉴 @img1 的空间形态、墙顶地关系、材质语言、灯光层次、构造细节和环境气质，并结合当前展馆类型、展厅主体及净高进行适配；不得复制其中原有展项、人物、文字或与当前主题无关的内容。'
      : '空间构建：没有空间参考图时，根据展馆类型、展厅主体及净高创建一个完整、自然、可施工的展厅空间。',
    `展厅净高：${safeHeight} mm。必须据此建立可信的空间尺度和展项尺度。`,
    floorConstraint,
    ceilingConstraint,
