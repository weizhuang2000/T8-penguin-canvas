...
function renderElevationTogetherText(values) {
  ...
  return [
    '【同时出立面】',
    '最终输出仍然只能是一张图：同一张画面内必须同时包含“室内透视效果图”和“对应正投影/平面立面图”，采用左右分区或上下分区的清晰拼版构图，两个分区边界明确、视觉风格统一。',
    '效果图分区继续严格遵守空间结构示意图、材质体系、工艺版式、展墙内容和展品呈现约束，保持真实室内建筑摄影级渲染。',
    '立面图分区参考“立面提示词”节点的彩立面与工艺排版逻辑：正投影表达、墙面边界清晰、图文层级明确，展板/灯箱/立体字/展柜/装置落位合理，材料和工艺可施工。',
