...
function renderElevationTogetherText(values) {
  if (values.renderElevationTogether !== true) return '';
  return [
    '【同时出立面】',
    '最终输出仍然只能是一张图：总画布为 1:1 方图、4K 输出，同一张画面内必须同时包含“室内透视效果图”和“对应正投影/平面立面图”。',
    '固定采用上下分区拼版：上半部分是室内透视效果图，下半部分是对应正投影/平面立面图；两个分区边界明确、上下对齐、视觉风格统一。',
    '效果图分区按 16:9 横向画面构图，继续严格遵守空间结构示意图、材质体系、工艺版式、展墙内容和展品呈现约束，保持真实室内建筑摄影级渲染。',
    '立面图分区也按 16:9 横向画面构图，参考“立面提示词”节点的彩立面与工艺排版逻辑：正投影表达、墙面边界清晰、图文层级明确，展板/灯箱/立体字/展柜/装置落位合理，材料和工艺可施工。',
    '立面图分区只表达当前效果图中对应的可见或指定立面范围；如果已有“展墙内容与设计（分立面执行）”，只选取与当前效果图匹配的立面内容，不重复输出其他立面。',
    '不得把立面拆成第二个文件或第二张输出；不得把同一工艺模块、展项模块或图文版式块在效果图分区和立面分区中重复堆叠表现；不得把同一工艺模块拆到多个立面区块中。',
  ].join('\n');
}
...
export function buildExhibitionImg2ImgPrompt(values = {}) {
  ...
  const renderElevationTogether = renderElevationTogetherText(values);
  if (renderElevationTogether) {
    lines.push(renderElevationTogether);
    lines.push('');
  }
  lines.push(
    '8. 最终输出约束（必读）',
