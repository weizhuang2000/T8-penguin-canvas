...
function baseCinemaContext(values = {}) {
  ...
  return [
    ...
    isFlyingCinema ? '飞行影院专用约束：空间必须表现为悬挂式或升降式飞行座舱剧场，不是普通地面座椅影院；前方为超大凹弧形/半包裹飞行视景屏，观众座舱由上方钢结构、升降臂或运动平台送入观影位置，下方保留装载等候、安全束缚、检修和特效设备区，座舱按多排/多组阵列排布并与风、雾、气味、震动等特效联动。' : '',
  ].join('\n');
}
...
export function buildCinemaAuditoriumImagePrompt(values = {}) {
  ...
  return [
    ...
    isFlyingCinema ? '飞行影院主效果图专用要求：必须表现悬挂式/升降式飞行影院，不要画成普通固定座椅影厅。画面应包含前方巨大凹弧形飞行视景屏或半包裹弧幕、上方钢结构轨道/升降臂/吊挂机构、多排悬挂座舱或运动座舱、下方装载等候与安全束缚区、风雾气味等特效设备暗示；观众座舱应像飞行模拟器座舱一样被抬升或悬挂面向弧幕。' : '',
...
export function buildCinemaAuditoriumDrawingPrompt(values = {}) {
  ...
  const typeRequirements = {
    'color-plan': isDomeCinema
      ...
      : isFlyingCinema
        ? '生成飞行影院彩色平面图：必须表现前方超大凹弧幕/半包裹飞行视景屏、悬挂/升降飞行座舱阵列、上方钢结构或升降臂轨道投影、装载等候区、安全束缚区、检修通道、控制机房、风雾气味等特效设备区和疏散方向。座舱按多排多组布置，不得画成普通地面座椅排布或报告厅平面。'
  ...
  const flyingTopologyText = isSystemPrinciple && isFlyingCinema
    ? '飞行影院设备拓扑必须额外细化：中央 show-control/PLC 安全控制 -> 升降臂/吊挂机构/运动平台控制柜 -> 多组飞行座舱执行器与安全锁扣 -> 座舱急停/限位/安全带检测；同时连接风机、雾化、气味、震动、音频、灯光和影片时间码同步系统，并标出装载区、检修区和应急下降/疏散联动。'
    : '';
