...
切换到远端空间后，新文件优先写入远端；仍标记为 `primary` 的旧文件会分批补传。只有远端副本确认成功并在本地保留满 7 天后，本地副本才会被清理。远端写入失败时，文件会保存在第一台 ECS，并在历史信息中显示回落状态。
...
- 当前保存空间为远端空间时，仍标记为 `primary` 的旧文件会按批次补传；默认每批最多 20 个，避免一次迁移占满磁盘和网络。
- 远端返回成功并完成摘要或大小校验后，才会把文件标记为远端副本，并从该时刻开始计算本地 7 天保留期。
- 远端上传失败、远端元数据未经确认、大小不一致或仍为 `primary` 的文件不会被本地清理。
- 服务启动时会安全检查一次；此后固定在北京时间每天 04:00 执行。每次都会输出扫描、候选、删除和各类跳过数量。
- 管理员可调用 `POST /api/output-storage/cleanup/preview` 预演；该接口只返回候选清单和统计，不删除文件。
- 管理员确认预演结果后可调用 `POST /api/output-storage/cleanup/run`，请求体必须为 `{ "confirm": true }`。

相关环境变量：

```text
T8_OUTPUT_STORAGE_SCAN_MS=30000
T8_OUTPUT_STORAGE_BACKFILL_BATCH_SIZE=20
T8_OUTPUT_STORAGE_LOCAL_RETENTION_MS=604800000
T8_OUTPUT_STORAGE_CLEANUP_HOUR=4
```
