...
function normalizeExhibitionCompactForm(raw = {}) {
  ...
  const incoming = raw && typeof raw === 'object' ? raw.sectionsByNodeType || {} : {};
  ...
  for (const definition of EXHIBITION_COMPACT_FORM_DEFINITIONS) {
    ...
    const seen = new Set();
    const normalized = [];
    for (const rawId of Array.isArray(incoming[definition.nodeType]) ? incoming[definition.nodeType] : fallback.sectionsByNodeType[definition.nodeType]) {
